...
const AutocompleteWrapper = styled(MuiAutocomplete)(({ theme }) => ({
  "& fieldset": {
    border: 0,
  },
    ...
    <Box sx={{ display: "flex", alignItems: "center", flexDirection: "column", justifyContent: "center" }}>
      ...
      <List sx={{ py: 0 }}>
        <ListItem sx={{ py: 2 }} disablePadding onClick={() => setOpenDialog(false)}>
          <RouterLink to="/dashboards/ecommerce/">
            <Box
              component={"a"}
              sx={{
                display: "flex",
        ...
        <ListItem sx={{ py: 2 }} disablePadding onClick={() => setOpenDialog(false)}>
          <RouterLink to="/apps/user/view/2/">
            <Box
              component={"a"}
              sx={{
                display: "flex",
        ...
        <ListItem sx={{ py: 2 }} disablePadding onClick={() => setOpenDialog(false)}>
          <RouterLink to="/pages/account-settings/">
            <Box
              component={"a"}
              sx={{
                display: "flex",
...
const Autocomplete = ({ hidden }) => {
  ...
  // Handle click event on a list sale in search result
  ...
  const handleKeydown = useCallback(
    (e) => {
      ...
      if (!openDialog && e.ctrlKey && e.which === 191) {
        setOpenDialog(true);
      }
    },
    [openDialog]
  ...
  return (
    ...
    >
      ...
      {!hidden && layout === "vertical" && <Typography sx={{ color: "text.disabled" }}>Search (Ctrl+/)</Typography>}
      <Dialog
        fullWidth
        ...
        <Box sx={{ top: 0, width: "100%", position: "sticky" }}>
          <AutocompleteWrapper
            ...
            renderOption={(props, option) => {
              const IconTag = autocompleteIconObj[option.icon] || THEME.NAV_SUB_ITEM_ICON;
